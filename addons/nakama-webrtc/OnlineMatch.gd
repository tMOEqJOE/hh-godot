...
func _create_webrtc_multiplayer() -> void:
	if _webrtc_multiplayer:
		get_tree().get_multiplayer().peer_connected.disconnect(self._on_webrtc_peer_connected)
		get_tree().get_multiplayer().peer_disconnected.disconnect(self._on_webrtc_peer_disconnected)

	_webrtc_multiplayer = WebRTCMultiplayerPeer.new()
	get_tree().get_multiplayer().peer_connected.connect(self._on_webrtc_peer_connected)
	get_tree().get_multiplayer().peer_disconnected.connect(self._on_webrtc_peer_disconnected)
...
func _on_nakama_match_state(data) -> void:
	...
	if data.op_code == MatchOpCode.JOIN_SUCCESS && _match_mode == MatchMode.JOIN:
		...
		if (not content_players.has(_my_session_id)):
			return
