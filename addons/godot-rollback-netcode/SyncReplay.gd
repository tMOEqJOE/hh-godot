...
func _do_setup_match1(my_peer_id: int, peer_ids: Array, match_info: Dictionary) -> void:
	...
	_scene_change_result = get_tree().change_scene_to_file(match_scene_path)
	...
	wait_another_frame.call_deferred(my_peer_id, peer_ids, match_info)

func wait_another_frame(my_peer_id: int, peer_ids: Array, match_info: Dictionary) -> void:
