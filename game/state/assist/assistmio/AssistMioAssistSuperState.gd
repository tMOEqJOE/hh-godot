...
class_name AssistMioSuperState
...
func physics_tick(state: Dictionary) -> void:
	...
	if (state[Enums.StKey.frame] == 14):
		state[Enums.StKey.velocity_x] = 0
	elif (state[Enums.StKey.frame] >= 8 and state[Enums.StKey.frame] <= 13):
		state[Enums.StKey.velocity_x] = SGFixed.ONE*250
