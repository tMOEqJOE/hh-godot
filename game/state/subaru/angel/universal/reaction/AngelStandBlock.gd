extends AngelReactionState

class_name AngelStandBlockState

const SLIDE_SPEED := -SGFixed.ONE*25
	...
	if (state[Enums.StKey.hitStopFrame] >= 0):
		if (burst_OK(state, interpreter)):
			...
			if (Util.assist_guard_cancel_exhausted(state)):
				state[Enums.StKey.cancelState] = "AngelAssistWeakGuardCancel"
			else:
				state[Enums.StKey.cancelState] = "AngelAssistGuardCancel"
			...
			change_state.call("AngelRedCrouchParryWhiff")
			...
			change_state.call("AngelRedStandParryWhiff")
