...
class_name OgajAirRiderKickLightState
...
func physics_tick(state: Dictionary) -> void:
	...
	if (state[Enums.StKey.frame] >= 6 and state[Enums.StKey.frame] < 12):
		state[Enums.StKey.velocity_y] = -SGFixed.ONE*35
		state[Enums.StKey.velocity_x] = Util.fixed_max(SGFixed.ONE*5, state[Enums.StKey.velocity_x])
