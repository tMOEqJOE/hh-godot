...
class_name OllieChargeBishopState
...
func handle_input(state: Dictionary, interpreter: InputInterpreter) -> void:
	...
	if (state[Enums.StKey.frame] == endFrame - 1):
		change_state.call("ChessBChargeThrow")
	if (not interpreter.is_button_down(Enums.InputFlags.BHold)):
		change_state.call("ChessBThrow")
