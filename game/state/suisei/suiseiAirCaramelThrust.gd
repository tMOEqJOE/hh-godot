...
class_name SuiseiAirCaramelThrustState
...
func enter(state: Dictionary) -> void:
	...
	state[Enums.StKey.velocity_y] = SGFixed.mul(state[Enums.StKey.velocity_y], 20536)
	state[Enums.StKey.accel_y] = 0

func physics_tick(state: Dictionary) -> void:
	super.physics_tick(state)
	if (state[Enums.StKey.frame] == 2):
		state[Enums.StKey.velocity_x] = Util.fixed_max(SGFixed.ONE*45, state[Enums.StKey.velocity_x])
